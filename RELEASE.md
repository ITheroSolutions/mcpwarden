...
```bash
# 0. Date the release in CHANGELOG.md: change "## [Unreleased]" to
#    "## [0.1.0] - YYYY-MM-DD" and commit. This comes first because
#    CHANGELOG.md ships inside the package, so it must be right before
...
# 6. Publish. This is the irreversible step. npm asks you to confirm in the
#    browser or for a one time code if your account has two factor enabled.
...
**On provenance.** A provenance statement is a signed record, shown on the npm
package page, linking the published package to this repository and the exact commit it
was built from. According to npm's documentation it can only be generated by a
supported CI provider such as GitHub Actions, which is why the local command above does
not pass `--provenance`.

To publish with provenance, use the `Release` GitHub Actions workflow instead. It is
`workflow_dispatch` only and defaults to a dry run. It needs an `NPM_TOKEN` repository
secret and an `npm-publish` environment, and neither exists yet. There is deliberately
no automatic path from a merge to a published package.
